...
public class productDao extends DBContext {
    ...
    public int countProductActive() {
        try {
            String sql = "select count(productID) as countProduct from Products1 where productStatus = 1";
    ...
    public List<Product> getNewProductTop3() {
        ...
        try {
            String sql = "select top 3 * from Products1 f \n"
    ...
    public List<Product> getNewProductTop6() {
        List<Product> product = new ArrayList<>();
        try {
            String sql = "select top 6 * from Products1 f \n"
                    + "left join category g on f.categoryID = g.categoryID \n";
            PreparedStatement stm = connection.prepareCall(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
    ...
    public List<Product> getAllProduct(String name) {
        List<Product> products = new ArrayList<>();
        try {
            String sql = "select * from Products1 f \n"
                    + " left join Category g on f.categoryID = g.categoryID \n"
    ...
    public List<Product> getProductByCondition(String proName, String category, int page, int page_Size) {
        List<Product> product = new ArrayList<>();
        try {
            String sql = "select * from Products1 f left join Category g on f.categoryID = g.categoryID \n"
    ...
    public int totalProductByCondition(String name, String category) {
        try {
            String sql = "select count(*) as count from Products1 f  left join Category g on f.categoryID = g.categoryID \n"
                    + "where productName like ? \n"
    ...
    public int registerProduct(int categoryID, String proName, String description, String image, float proPrice) {
        try {
            String sql = "INSERT INTO [dbo].[Products1]\n"
                    + "           ([categoryID]\n"
                    + "           ,[productName]\n"
                    + "           ,[descript]\n"
                    + "           ,[imagee]\n"
    ...
    public int deleteProduct(int proId) {
        try {
            String sql = "DELETE FROM [dbo].[Products1] \n"
                    + "      WHERE productId = ?";
            PreparedStatement stm = connection.prepareCall(sql);
            stm.setInt(1, proId);
            stm.executeUpdate();
    ...
    public int updateStatusProduct(int productId) {
        try {
            String sql = "UPDATE [dbo].[Products1]\n"
                    + "   SET [productStatus] = 0\n"
                    + " WHERE productID = ?";
            PreparedStatement stm = connection.prepareCall(sql);
            stm.setInt(1, productId);
